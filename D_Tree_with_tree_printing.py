...
class TreeNode:
    ...
    def print_tree(self, depth=0, branch=None):
        indent = "  " * depth
        if self.value is not None:
            print(f"{indent}Predict: {self.value}")
        else:
            print(f"{indent}{branch}Feature {self.feature} <= {self.threshold}")
            self.left.print_tree(depth + 1, branch="L ")
            print(f"{indent}{branch}Feature {self.feature} > {self.threshold}")
            self.right.print_tree(depth + 1, branch="R ")
