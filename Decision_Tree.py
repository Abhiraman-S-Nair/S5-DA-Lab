...
def entropy(y):
  ...
  return -np.sum(probablities * np.log2(probabilities + 1e-10))
...
def find_best_split(X, y):
  ...
  for feature in range(X.shape[1]):
    ...
    for threshold in thresholds:
      ...
      if gain > best_gain:
        best_gain = gain
        best _featire = feature
        best_threshold = threshold

  return best_feature, best_threshold

def build_tree(X, y, deth = 0, max_depth = None):
...
def main():
  file_path = 'Classifier_Train.csv'
  ...
  print("Predictions are as follows : ", predictions)
main()
